"""Per-task logging utility (one file per task, flat ``output CreditPFN/logs/`` dir).

This is the project's filled-in version of the template's `logging_setup.py` slot: same
role — one logger, console **and** `output CreditPFN/logs/` — with the two things this project needs
...
**exactly one** log file with the naming convention:
    ...
    output CreditPFN/logs/<task>_<YYYYMMDD>_<HHMMSS>.log
...
the basename to keep array tasks distinct:
    ...
    output CreditPFN/logs/train_pd_<YYYYMMDD>_<HHMMSS>_j<JOBID>_a<TASKID>.log

The log file lives under ``$CREDITPFN_OUTPUT_ROOT/output CreditPFN/logs/`` (on VSC =
``$VSC_DATA/CreditPFN/output CreditPFN/logs``, locally = the repo's ``output CreditPFN/logs/``).
