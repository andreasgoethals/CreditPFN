...
- **Output directory:** `output/<experiment>/` adds an experiment layer to the template. Use `src/utils/paths.py` on both storage tiers. Detailed training diagnostics and results go to project storage; logs/small manifests to DATA. Weights retain `checkpoints/trained/<experiment>/`.
- **Keep cluster debugging on VSC.** Read Downloads in place when asked; do not copy its files
  ...
  `output/general/All_Results.md`; no duplicate notebook logs or locks. Caption metadata remains in
  `output/<experiment>/manifests/figures/`. Give VSC commands in ordinary messages, one command per code block.
