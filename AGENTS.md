...
- **Output directory:** `output CreditPFN/` deliberately replaces the generic template's `output/`; use `src/utils/paths.py` on both storage tiers. Weights keep the separate `checkpoints/` location.
- **Keep cluster debugging on VSC.** Read Downloads in place when asked; do not copy its files
  ...
  `output CreditPFN/manifests/figures/`. Give VSC commands in ordinary messages, one command per code block.
